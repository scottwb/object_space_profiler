# Slightly modified version of the MemoryProfiler script found at:
#
...
class ObjectMemoryProfiler
  ...
  def self.start(opt={})
    ...
    Thread.new do
      ...
      file = File.open('log/obj_mem_profiler.log','w')
      ...
      loop do
        begin
          ...
          if opt[:string_debug]
            File.open("log/obj_mem_profiler_strings.log.#{Time.now.to_i}",'w') do |f|
          ...
          STDERR.puts "** ObjectMemoryprofiler error: #{err}"
        end
        sleep opt[:delay]
      end
    end
